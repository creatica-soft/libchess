...
if __name__ == "__main__":
  ...
  pgnDir = '/Users/ap/Downloads/Lichess Elite Database/'
  ...
  #pgnFilesTrain = ['KingBaseLite2019-01.pgn']
  #dataset = datasetTrain # datasetTrain, datasetTest or None
  #pgnFiles = pgnFilesTrain # pgnFilesTrain or pgnFilesTest
  pgn_dir = Path(pgnDir)
  pgnFilesList = [ffi.new("char[]", (str(s)).encode()) for s in pgn_dir.glob("lichess_elite_*.pgn") if s.is_file()]
  ...
  num = 0;
  start = datetime.now()
  res = lib.openGamesFromPGNfiles(fileNames, len(pgnFilesList), threads, sql_threads, ecoFileName, minElo, maxEloDiff, minMoves, numberOfGames, generateZobristHash, updateDb, createDataset, ds, engineEval, engine, movetime, depth, hashSize, engineThreads, syzygy, multiPV)
  num += res
  if ds != ffi.NULL:
    ffi.release(ds) 
