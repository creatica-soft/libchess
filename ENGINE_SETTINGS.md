...
| Why does `runMCTS` run twice for some moves? | instrument `searchFlag`/`run_go` entry and log every transition | measured at 1.7-4.3% of moves in live games: a second complete search runs and a second `bestmove` is emitted for one `go`. `engine.cpp` now drains stale output so the driver is unharmed, but an engine emitting two bestmoves violates UCI and any other GUI would desynchronise. Suspected in the ponder path, where `go ponder` and the following real `go` may both reach `runMCTS` |
