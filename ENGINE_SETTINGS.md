...
| What actually caused the 60 s engine stall? | reproduce with per-engine logs now that they exist | a live game showed `engineFgets` declaring the engine dead, then an empty move POSTed to lichess (404). The duplicated `bestmove` lines first suspected were an artefact of two bots sharing one log. The remaining candidate is collection time: the same log shows 199 collections averaging 773 ms with a **maximum of 11.9 s**, on a tree that reached 8.3M nodes at 742 per-mille |
