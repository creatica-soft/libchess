...
// A single Residual Block: Input -> Conv -> BN -> ReLU -> Conv -> BN + Input -> ReLU
struct ResBlockImpl : torch::nn::Module {
    ...
    ResBlockImpl(int channels) {
        // Preserves dimensions (3x3 kernel, stride 1, padding 1)
        conv1 = register_module("conv1", torch::nn::Conv2d(torch::nn::Conv2dOptions(channels, channels, 3).padding(1).stride(1).bias(false)));
        bn1 = register_module("bn1", torch::nn::BatchNorm2d(channels));
        conv2 = register_module("conv2", torch::nn::Conv2d(torch::nn::Conv2dOptions(channels, channels, 3).padding(1).stride(1).bias(false)));
...
struct ResNetCNNImpl : torch::nn::Module {
    // Initial processing
    torch::nn::Conv2d conv_input{nullptr};
    torch::nn::BatchNorm2d bn_input{nullptr};
    //torch::nn::Conv2d conv2_input{nullptr};
    //torch::nn::BatchNorm2d bn2_input{nullptr};
    
    // The "Tower" of residual blocks
    torch::nn::Sequential tower{nullptr};
    torch::nn::BatchNorm2d head_norm{nullptr};
    
    // Value Head (The output you are currently using)
    ...
    ResNetCNNImpl() {
        // 1. Input Layer: 15 input planes -> 64 filters
        conv_input = register_module("conv_input", torch::nn::Conv2d(torch::nn::Conv2dOptions(15, 128, 3).padding(1)));
        bn_input = register_module("bn_input", torch::nn::BatchNorm2d(128));

        //conv2_input = register_module("conv2_input", torch::nn::Conv2d(torch::nn::Conv2dOptions(32, 64, 3).padding(1)));
        //bn2_input = register_module("bn2_input", torch::nn::BatchNorm2d(64));

        // 2. Residual Tower (5 Blocks is a good starting point for CPU)
        tower = register_module("tower", torch::nn::Sequential());
        for (int i = 0; i < 3; ++i) {
            tower->push_back(ResBlock(128));
        }
        head_norm = register_module("head_norm", torch::nn::BatchNorm2d(128));
        // 3. Value Head
        // Reduce 64 filters to 1 filter (1x8x8)
        conv_val = register_module("conv_val", torch::nn::Conv2d(torch::nn::Conv2dOptions(128, 64, 1))); 
        bn_val = register_module("bn_val", torch::nn::BatchNorm2d(64));
        
        // Fully connected: 64 squares -> 64 hidden -> 1 output
        fc_val1 = register_module("fc_val1", torch::nn::Linear(4096, 128)); //was 64
        fc_val2 = register_module("fc_val2", torch::nn::Linear(128, 1));
    }

    torch::Tensor forward(torch::Tensor x) {
        // Input processing
        x = torch::relu(bn_input(conv_input(x)));
        //x = torch::relu(bn2_input(conv2_input(x)));
        ...
        // Value Head
        x = torch::relu(bn_val(conv_val(x))); 
        x = x.view({x.size(0), -1}); // Flatten 1x8x8 -> 64
        x = torch::relu(fc_val1(x));
        x = torch::tanh(fc_val2(x)); // Output [-1, 1]
        
        return x;
    }
};
TORCH_MODULE(ResNetCNN);
/*
void init_resnet_weights(ResNetCNN& model) {
    torch::NoGradGuard no_grad;

    // 1. Initialize Input Layer
    torch::nn::init::kaiming_normal_(model->conv_input->weight, 0.0, torch::kFanIn, torch::kReLU);
    torch::nn::init::kaiming_normal_(model->conv2_input->weight, 0.0, torch::kFanIn, torch::kReLU);

    // 2. Initialize Tower
    for (auto& module : *model->tower) {
        // Cast the generic module back to our ResBlock
        auto block = module.ptr<ResBlockImpl>();
        
        // standard Kaiming for convolutions
        torch::nn::init::kaiming_normal_(block->conv1->weight, 0.0, torch::kFanIn, torch::kReLU);
        torch::nn::init::kaiming_normal_(block->conv2->weight, 0.0, torch::kFanIn, torch::kReLU);
        
        // Zero-Init trick: Final BN of the block starts at 0
        torch::nn::init::constant_(block->bn1->weight, 1.0);
        torch::nn::init::constant_(block->bn2->weight, 0.0);
        torch::nn::init::constant_(block->bn1->bias, 0.0);
        torch::nn::init::constant_(block->bn2->bias, 0.0);
    }

    // 3. Initialize Value Head
    torch::nn::init::kaiming_normal_(model->conv_val->weight);
    torch::nn::init::kaiming_normal_(model->fc_val1->weight);
    model->fc_val1->weight.data().mul_(0.01); // Scale down by 100x
    torch::nn::init::constant_(model->fc_val1->bias, 0.0);
    // Xavier uniform for the final tanh-bound output
    torch::nn::init::xavier_uniform_(model->fc_val2->weight);
    torch::nn::init::constant_(model->fc_val2->bias, 0.0);
}*/
...
public:
  ...
  /*
        ...
        //normalized_score /= 3900.0f; // Range -1.0 to 1.0; negative score - white's loosing, positive - white's winning 
        //instead of linear clamping, use sigmoid
        float normalized_score = tanh((float)pos.eval_cp / 800.0f);
...
// 5. Create Target: Since board is oriented for the mover,
// the target is always "How good is this for the mover?"
float score_for_mover = is_black ? -(float)pos.eval_cp : (float)pos.eval_cp;
float normalized_score = std::tanh(score_for_mover / 800.0f);
...
int main() {
    ...
    const std::string weights_file = "resnet_cnn.pt";
    ...
    if (std::filesystem::exists(weights_file)) {
        ...
        //init_resnet_weights(model); //default init is much better! this one makes the model unstable
    ...
    for (int epoch = 1; epoch <= num_epochs; ++epoch) {        
        ...
        for (const auto& filepath : file_list) {
            ...
            // Save Checkpoint
            torch::save(model, "resnet_cnn_checkpoint_" + std::to_string(epoch) + ".pt");            
        }
        ...
        if (std::filesystem::exists(test_data_path)) {
            ...
            { // Scope for memory management
                ...
                for (auto& batch : *test_loader) {
                    ...
                    auto targets = batch.target.to(device).to(torch::kHalf);

                    auto output = model->forward(data);
                    auto loss = torch::mse_loss(output, targets).to(torch::kFloat);
