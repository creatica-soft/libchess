...
class FeatureTransformer {
   ...
   public:
    ...
    static constexpr IndexType TotalInputDimensions = InputDimensions + (UseThreats ? ThreatInputDimensions : 0);
    ...
    static constexpr std::uint32_t get_hash_value() {
        return (UseThreats ? ThreatFeatureSet::HashValue : PSQFeatureSet::HashValue) ^ (OutputDimensions * 2);
    }
...
std::int32_t transform(const Board& board, AccumulatorStack& accumulatorStack, AccumulatorCaches::Cache<HalfDimensions>& cache, OutputType* output, int bucket) const {
        using namespace SIMD;
        accumulatorStack.evaluate(board, *this, cache); //see nnue_accumulator.cpp
        const auto& accumulatorState = accumulatorStack.latest<PSQFeatureSet>(); //see nnue_accumulator.cpp
        const auto& threatAccumulatorState = accumulatorStack.latest<ThreatFeatureSet>();

        const Color perspectives[2] = {static_cast<Color>(board.sideToMove), static_cast<Color>(board.sideToMove ^ 1)};
        const auto& psqtAccumulation = (accumulatorState.acc<HalfDimensions>()).psqtAccumulation;
        auto psqt = (psqtAccumulation[perspectives[0]][bucket] - psqtAccumulation[perspectives[1]][bucket]);
        //printf("NNUE::transform() debug: psqt %d, bucket %d\n", psqt, bucket);
        if constexpr (UseThreats) {
            const auto& threatPsqtAccumulation = (threatAccumulatorState.acc<HalfDimensions>()).psqtAccumulation;
            psqt = (psqt + threatPsqtAccumulation[perspectives[0]][bucket] - threatPsqtAccumulation[perspectives[1]][bucket]) / 2;
        } else psqt /= 2;

        const auto& accumulation = (accumulatorState.acc<HalfDimensions>()).accumulation;
        const auto& threatAccumulation = (threatAccumulatorState.acc<HalfDimensions>()).accumulation;
        ...
        for (IndexType p = 0; p < 2; ++p) {
            ...
            for (IndexType j = 0; j < HalfDimensions / 2; ++j)
            {
                ...
                if constexpr (UseThreats) {
                    BiasType sum0t = threatAccumulation[static_cast<int>(perspectives[p])][j + 0];
                    BiasType sum1t = threatAccumulation[static_cast<int>(perspectives[p])][j + HalfDimensions / 2];
                    sum0 = std::clamp<BiasType>(sum0 + sum0t, 0, 255);
                    sum1 = std::clamp<BiasType>(sum1 + sum1t, 0, 255);
                } else {
                    sum0 = std::clamp<BiasType>(sum0, 0, 127 * 2);
    ...
    alignas(CacheLineSize) std::array<ThreatWeightType, UseThreats ? HalfDimensions * ThreatInputDimensions : 0> threatWeights;
    alignas(CacheLineSize) std::array<PSQTWeightType, InputDimensions * PSQTBuckets> psqtWeights;
    alignas(CacheLineSize) std::array<PSQTWeightType, UseThreats ? ThreatInputDimensions * PSQTBuckets : 0> threatPsqtWeights;
};
