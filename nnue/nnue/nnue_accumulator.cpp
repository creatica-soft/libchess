...
template<IndexType Dimensions>
void AccumulatorStack::evaluate(const Board& board, const FeatureTransformer<Dimensions>& featureTransformer, AccumulatorCaches::Cache<Dimensions>& cache) noexcept {
    constexpr bool UseThreats = (Dimensions == TransformedFeatureDimensionsBig);
    evaluate_side<PSQFeatureSet>(WHITE, board, featureTransformer, cache);
    if (UseThreats) evaluate_side<ThreatFeatureSet>(WHITE, board, featureTransformer, cache);
    evaluate_side<PSQFeatureSet>(BLACK, board, featureTransformer, cache);
    if (UseThreats) evaluate_side<ThreatFeatureSet>(BLACK, board, featureTransformer, cache);
}
...
template<typename FeatureSet, IndexType Dimensions>
void AccumulatorStack::evaluate_side(Color perspective, const Board& board, const FeatureTransformer<Dimensions>& featureTransformer, AccumulatorCaches::Cache<Dimensions>& cache) noexcept {
    const auto last_usable_accum = find_last_usable_accumulator<FeatureSet, Dimensions>(perspective);
    if ((accumulators<FeatureSet>()[last_usable_accum].template acc<Dimensions>()).computed[perspective])
        forward_update_incremental<FeatureSet>(perspective, board, featureTransformer, last_usable_accum);
    else {
        if constexpr (std::is_same_v<FeatureSet, PSQFeatureSet>)
            update_accumulator_refresh_cache(perspective, featureTransformer, board, mut_latest<PSQFeatureSet>(), cache);
        else
            update_threats_accumulator_full(perspective, featureTransformer, board, mut_latest<ThreatFeatureSet>());
        backward_update_incremental<FeatureSet>(perspective, board, featureTransformer, last_usable_accum);
    }
}
...
template<typename FeatureSet, IndexType Dimensions>
void AccumulatorStack::forward_update_incremental(Color perspective, const Board& board, const FeatureTransformer<Dimensions>& featureTransformer, const std::size_t begin) noexcept {
    assert(begin < accumulators<FeatureSet>().size());
    assert((accumulators<FeatureSet>()[begin].template acc<Dimensions>()).computed[perspective]);
    //const Square ksq = pos.square<KING>(perspective);
    const Square ksq = lsb(board.side[perspective] & board.pieceTypes[KING - 1]);
    ...
    for (std::size_t next = begin + 1; next < size; next++) {
        if (next + 1 < size) {
            ...
            auto& accumulators = mut_accumulators<FeatureSet>();
            if constexpr (std::is_same_v<FeatureSet, ThreatFeatureSet>) {
                if (dp2.remove_sq != SQ_NONE && (accumulators[next].diff.threateningSqs & square_bb(dp2.remove_sq))) {
                    double_inc_update(perspective, featureTransformer, ksq, accumulators[next], accumulators[next + 1], accumulators[next - 1], dp2);
                    next++;
                    continue;
                }
            }
...
template<typename FeatureSet, IndexType Dimensions>
void AccumulatorStack::backward_update_incremental(Color perspective, const Board& board, const FeatureTransformer<Dimensions>& featureTransformer, const std::size_t end) noexcept {
    assert(end < accumulators<FeatureSet>().size());
    assert(end < size);
    assert((latest<FeatureSet>().template acc<Dimensions>()).computed[perspective]);
    //const Square ksq = pos.square<KING>(perspective);
    const Square ksq = lsb(board.side[perspective] & board.pieceTypes[KING - 1]);
    for (std::int64_t next = std::int64_t(size) - 2; next >= std::int64_t(end); next--)
        update_accumulator_incremental<false>(perspective, featureTransformer, ksq, mut_accumulators<FeatureSet>()[next], accumulators<FeatureSet>()[next + 1]);
    assert((accumulators<FeatureSet>()[end].template acc<Dimensions>()).computed[perspective]);
}
...
template void AccumulatorStack::evaluate<TransformedFeatureDimensionsBig>(const Board& board, const FeatureTransformer<TransformedFeatureDimensionsBig>& featureTransformer, AccumulatorCaches::Cache<TransformedFeatureDimensionsBig>& cache) noexcept;
template void AccumulatorStack::evaluate<TransformedFeatureDimensionsSmall>(const Board& board, const FeatureTransformer<TransformedFeatureDimensionsSmall>& featureTransformer, AccumulatorCaches::Cache<TransformedFeatureDimensionsSmall>& cache) noexcept; 
...
Bitboard get_changed_pieces(const std::array<Piece, SQUARE_NB>& oldPieces, const std::array<Piece, SQUARE_NB>& newPieces) {
...
void update_accumulator_refresh_cache(Color                                 perspective,
    ...
    const Bitboard changedBB = (entry.pieces, pos.piece_array());
...
template<IndexType Dimensions>
void update_accumulator_refresh_cache(Color perspective, const FeatureTransformer<Dimensions>& featureTransformer, const Board& board, AccumulatorState<PSQFeatureSet>& accumulatorState, AccumulatorCaches::Cache<Dimensions>& cache) {
    using Tiling [[maybe_unused]] = SIMDTiling<Dimensions, Dimensions, PSQTBuckets>;
    //const Square             ksq   = pos.square<KING>(perspective);
    const Square ksq = lsb(board.side[perspective] & board.pieceTypes[KING - 1]);
    auto& entry = cache[ksq][perspective];
    ...
    //printf("update_accumulator_refresh_cache() debug: board.piecesOnSquares\n");
    //for (auto& p : board.piecesOnSquares) printf("%d ", p);
    //printf("\n");

    //const Bitboard changedBB = get_changed_pieces(entry.pieces, pos.piece_array());
    const Bitboard changedBB = get_changed_pieces(entry.pieces, *reinterpret_cast<const std::array<Piece, 64>*>(board.piecesOnSquares));
    //printf("update_accumulator_refresh_cache() debug: entry.pieceBB %llx, changedBB %llx\n", entry.pieceBB, changedBB);
    Bitboard removedBB = changedBB & entry.pieceBB;
    //Bitboard       addedBB   = changedBB & pos.pieces();
    Bitboard addedBB  = changedBB & (board.side[WHITE] | board.side[BLACK]);
    ...
    while (addedBB) {
        Square sq = pop_lsb(addedBB);
        //added.push_back(PSQFeatureSet::make_index(perspective, sq, pos.piece_on(sq), ksq));
        added.push_back(PSQFeatureSet::make_index(perspective, sq, static_cast<Piece>(board.piecesOnSquares[sq]), ksq));
    }
    //printf("update_accumulator_refresh_cache() debug: removed %d, added %d\n", removed.ssize(), added.ssize());
    
    //entry.pieceBB = pos.pieces();
    //entry.pieces  = pos.piece_array();
    entry.pieceBB = board.side[WHITE] | board.side[BLACK];
    entry.pieces = *reinterpret_cast<const std::array<Piece, 64>*>(board.piecesOnSquares);
    ...
    /*for (Color c : {WHITE, BLACK})
    {
        for (PieceType pt = PAWN; pt <= KING; ++pt)
        {
            ...
            const Bitboard oldBB    = entry.pieces[c];
            const Bitboard newBB    = board.side[c] & board.pieceTypes[pt - 1];
    ...
    for (IndexType j = 0; j < PSQTBuckets / Tiling::PsqtTileHeight; ++j) {
        auto* accTilePsqt = reinterpret_cast<psqt_vec_t*>(&accumulator.psqtAccumulation[perspective][j * Tiling::PsqtTileHeight]);
        auto* entryTilePsqt = reinterpret_cast<psqt_vec_t*>(&entry.psqtAccumulation[j * Tiling::PsqtTileHeight]);
        for (IndexType k = 0; k < Tiling::NumPsqtRegs; ++k)
            psqt[k] = entryTilePsqt[k];
        for (int i = 0; i < removed.ssize(); ++i) {
            size_t index = removed[i];
            const size_t offset = PSQTBuckets * index + j * Tiling::PsqtTileHeight;
            auto* columnPsqt = reinterpret_cast<const psqt_vec_t*>(&featureTransformer.psqtWeights[offset]);
            for (std::size_t k = 0; k < Tiling::NumPsqtRegs; ++k)
                psqt[k] = vec_sub_psqt_32(psqt[k], columnPsqt[k]);
        }
        for (int i = 0; i < added.ssize(); ++i) {
...
template<IndexType Dimensions>
void update_threats_accumulator_full(Color perspective, const FeatureTransformer<Dimensions>& featureTransformer, const Board& board,
 AccumulatorState<ThreatFeatureSet>& accumulatorState) {
    using Tiling [[maybe_unused]] = SIMDTiling<Dimensions, Dimensions, PSQTBuckets>;
    ThreatFeatureSet::IndexList active;
    ThreatFeatureSet::append_active_indices(perspective, board, active); //see full_threats.cpp
    //printf("update_threats_accumulator_full() debug: active indeces number %d, Dimensions %d, PSQTBuckets %d\n", active.ssize(), Dimensions, PSQTBuckets);
    auto& accumulator = accumulatorState.acc<Dimensions>();
    accumulator.computed[perspective] = true;
#ifdef VECTOR
    //printf("update_threats_accumulator_full() debug: VECTOR defined\n");
    vec_t      acc[Tiling::NumRegs];
    psqt_vec_t psqt[Tiling::NumPsqtRegs];
    const auto* threatWeights = &featureTransformer.threatWeights[0];
    for (IndexType j = 0; j < Dimensions / Tiling::TileHeight; ++j) {
        ...
        for (; i < active.ssize(); ++i) {
            ...
            auto*        column = reinterpret_cast<const vec_i8_t*>(&threatWeights[offset]);
    #ifdef USE_NEON
            //printf("update_threats_accumulator_full() debug: USE_NEON defined\n");
            for (IndexType k = 0; k < Tiling::NumRegs; k += 2) {
                acc[k]     = vec_add_16(acc[k], vmovl_s8(vget_low_s8(column[k / 2])));
                acc[k + 1] = vec_add_16(acc[k + 1], vmovl_high_s8(column[k / 2]));
            }
    #else
            //printf("update_threats_accumulator_full() debug: USE_NEON NOT defined\n");
            for (IndexType k = 0; k < Tiling::NumRegs; ++k)
                acc[k] = vec_add_16(acc[k], vec_convert_8_16(column[k]));
    #endif
        }
...
#else
    //printf("update_threats_accumulator_full() debug: VECTOR NOT defined. Dimensions %d, buckets %d\n", Dimensions, PSQTBuckets);
    for (IndexType j = 0; j < Dimensions; ++j)
        accumulator.accumulation[perspective][j] = 0;
    for (std::size_t k = 0; k < PSQTBuckets; ++k)
        accumulator.psqtAccumulation[perspective][k] = 0;
    for (const auto index : active) {
        const IndexType offset = Dimensions * index;
        for (IndexType j = 0; j < Dimensions; ++j)
            accumulator.accumulation[perspective][j] += featureTransformer.threatWeights[offset + j];
        for (std::size_t k = 0; k < PSQTBuckets; ++k)
            accumulator.psqtAccumulation[perspective][k] += featureTransformer.threatPsqtWeights[index * PSQTBuckets + k];
    }
#endif
}
}
