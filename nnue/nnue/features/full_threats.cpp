    ...
    for (Piece piece : AllPieces) {
        ...
        for (Square from = SQ_A1; from <= SQ_H8; ++from) {
            ...
            if (type_of(piece) != PAWN) {
                ...
                Bitboard attacks = (pieceIdx < 8) ? pawn_attacks_bb<WHITE>(square_bb(from)) : pawn_attacks_bb<BLACK>(square_bb(from));
                cumulativePieceOffset += constexpr_popcount(attacks);
            }
        }
...
constexpr auto init_index_luts() {
    ...
    for (Piece attacker : AllPieces) {
        for (Piece attacked : AllPieces) {
            ...
            IndexType feature = helper_offsets[attacker].cumulativeOffset + (color_of(attacked) * (numValidTargets[attacker] / 2) + map) * helper_offsets[attacker].cumulativePieceOffset;
            bool excluded = map < 0;
            indices[attacker][attacked][0] = excluded ? FullThreats::Dimensions : feature;
            indices[attacker][attacked][1] = excluded || semi_excluded ? FullThreats::Dimensions : feature;
        }
    }
    return indices;
}
...
void FullThreats::append_active_indices(Color perspective, const Board& board, IndexList& active) {
    //Square   ksq      = pos.square<KING>(perspective);
    Square ksq = lsb(board.side[perspective] & board.pieceTypes[KING - 1]);
    //Bitboard occupied = pos.pieces();
    Bitboard occupied = board.side[WHITE] | board.side[BLACK];
    ...
    for (Color color : {WHITE, BLACK}) {
        for (PieceType pt = PAWN; pt <= KING; ++pt) {
            ...
            Piece attacker = make_piece(c, pt);
            //Bitboard bb       = pos.pieces(c, pt);
            Bitboard bb = board.side[c] & board.pieceTypes[pt - 1];
            if (pt == PAWN) {
                auto right = (c == WHITE) ? NORTH_EAST : SOUTH_WEST;
                auto left = (c == WHITE) ? NORTH_WEST : SOUTH_EAST;
                ...
                while (attacks_left) {
                    ...
                    Square from = to - right;
                    //Piece     attacked = pos.piece_on(to);
                    Piece attacked = static_cast<Piece>(board.piecesOnSquares[to]);
                    IndexType index = make_index(perspective, attacker, from, to, attacked, ksq);
                    if (index < Dimensions) active.push_back(index);
                }
                while (attacks_right){
                    Square to = pop_lsb(attacks_right);
                    Square from = to - left;
...
void FullThreats::append_changed_indices(Color perspective, Square ksq, const DiffType&  diff, IndexList& removed, IndexList& added, FusedUpdateData* fusedData, bool first) {
