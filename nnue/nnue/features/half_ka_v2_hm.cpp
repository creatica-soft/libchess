...
void HalfKAv2_hm::append_active_indices(Color perspective, const Board& board, IndexList& active) {
    //Square   ksq = pos.square<KING>(perspective);
    Square ksq = lsb(board.side[perspective] & board.pieceTypes[KING - 1]);
    //Bitboard bb  = pos.pieces();
    Bitboard bb = board.side[WHITE] | board.side[BLACK];
    while (bb)
    {
        Square s = pop_lsb(bb);
        //active.push_back(make_index(perspective, s, pos.piece_on(s), ksq));
        active.push_back(make_index(perspective, s, static_cast<Piece>(board.piecesOnSquares[s]), ksq));
    }
}
