...
//c++ -O3 -I /Users/ap/Downloads/libtorch/include -I /Users/ap/Downloads/libtorch/include/torch/csrc/api/include -L /Users/ap/Downloads/libtorch/lib -L /Users/ap/libchess -std=c++17 -Wl,-ltorch,-ltorch_cpu,-lc10,-lchess,-rpath,/Users/ap/Downloads/libtorch/lib,-rpath,/Users/ap/libchess -o chess_cnn5 train_chess_cnn5.cpp chess_cnn5.cpp
...
for (int64_t i = 0; i < batch.board_moves.size(0); i++) {
  ...
  std::cerr << batch.stage.index({i}) << std::endl;
        ...
        for (int s = 0; s < 3; ++s) {
            ...
            std::cerr << "Stage " << gameStage[s] << ": " << mask_sum / batch.board_moves.size(0) * 100 << "%" << std::endl;
            ...
            if (mask_sum > 0) {
                ...
                stage_loss = (p_loss.sum() + 0.2 * v_loss.sum()) / mask_sum;
    ...
    while (true) {
        ...
        for (int s = 0; s < 3; ++s) {
            ...
            if (mask_sum > 0) {
                ...
                stage_loss = (p_loss.sum() + v_loss.sum()) / mask_sum;
                policy_loss += p_loss.sum() / mask_sum;
                value_loss += v_loss.sum() / mask_sum;
                // Compute accuracy
...
int main(int argc, char ** argv) {
    ...
    const std::string weights_file = "chessCNN5_1.pt";
