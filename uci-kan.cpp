...
void search_thread_func(EvalQueue<65536>& queue) {
    while (!quitFlag.load()) {
        ...
        if (searchFlag.load()) runMCTS(queue);
        else continue;
        lock.lock();
          ...
          if (v >= chessEngine.optionSpin[idx].min && v <= chessEngine.optionSpin[idx].max) {
            ...
            switch(idx) {
              ...
              case VirtualLoss:
                ...
                virtual_loss = std::tanh(static_cast<double>(chessEngine.optionSpin[VirtualLoss].value) / eval_scale);
                break;
              //case EvalScale:
...
void setEngineOptions() {
    strcpy(chessEngine.id, "Creatica Bipolar ChebKAN 1.0");
...
// Search thread function
void persistent_worker_func(int thread_id, EvalQueue<65536>& queue) {
...
void init_thread_pool(int num_threads, EvalQueue<65536>& queue) {
   ...
   // Stop existing threads if any
    ...
    for (int i = 0; i < num_threads; ++i)  {
      pool_params[i].mt = std::mersenne_twister_engine<std::uint_fast32_t, 32, 624, 397, 31,
                             0x9908b0df, 11,
                             0xffffffff, 7,
                             0x9d2c5680, 15,
                             0xefc60000, 18, 1812433253>(rd());
      pool_threads.emplace_back(persistent_worker_func, i, std::ref(queue));
    }
}
...
int main(int argc, char **argv) {
    ...
    if (torch::cuda::is_available()) {
        ...
        log_file("Using MPS device\n");        
    } else {
        log_file("Neither CUDA nor MPS available. Using CPU\n");
    }
    ...
    if (std::filesystem::exists(weights_file)) {
      ...
      log_file("Loaded weights from %s\n", weights_file.c_str());
        ...
        log_file("Weights file %s not found, exiting...\n", weights_file.c_str());
        return 1;
    }
    ...
    // 1. Allocate the queue on the heap for strict 128-byte alignment
    auto queue = std::make_unique<EvalQueue<65536>>();

    // 2. Initialize the PredictionServer
    PredictionServer server(*queue, model);

    // 3. Start the server in a separate thread
    // We use a lambda to call the run() method
    std::thread server_thread([&server]() {
        server.run();
    });

    // 4. Start your Search Threads
    init_thread_pool(chessEngine.optionSpin[Threads].value, *queue);

    std::thread search_thread(search_thread_func, std::ref(*queue));
    if (argc == 2 && std::string(argv[1]) == "bench") {
      ...
      handleQuit();
    } else if (argc == 2 && std::string(argv[1]) == "export") {
      handleQuit();
    }
    else {
      uciLoop();
