...
ChessCNNImpl::ChessCNNImpl() {
    ...
    board_conv = register_module("board_conv", torch::nn::Sequential(
        torch::nn::Conv2d(torch::nn::Conv2dOptions(2, 64, 7).padding(3)),    
        torch::nn::ReLU(),
        ResidualBlock(64),
        ResidualBlock(64),
        ResidualBlock(64),
    ...
    legal_conv = register_module("legal_conv", torch::nn::Sequential(
        torch::nn::Conv2d(torch::nn::Conv2dOptions(16, 64, 1)),
        torch::nn::ReLU(),
        ResidualBlock(64)
    ));
    ...
    policy_promo_opening = register_module("policy_promo_opening", torch::nn::Sequential(
        torch::nn::Conv2d(torch::nn::Conv2dOptions(128, 16, 1)),
        torch::nn::Flatten(),
    ...
    value_head_opening = register_module("value_head_opening", torch::nn::Sequential(
        torch::nn::Conv2d(torch::nn::Conv2dOptions(128, 8, 1)),
        torch::nn::Flatten(),
    ...
    policy_promo_middlegame = register_module("policy_promo_middlegame", torch::nn::Sequential(
        torch::nn::Conv2d(torch::nn::Conv2dOptions(512, 16, 1)),
        torch::nn::Flatten(),
        torch::nn::Linear(1024, 64),
        torch::nn::ReLU(),
        torch::nn::Linear(64, 4)
...
std::tuple<torch::Tensor, torch::Tensor,
    ...
    // Split input: first 2 channels for board, remaining 64 for legal moves, total 66 channels
    auto x_board = x.slice(1, 0, 2);
    auto x_legal = x.slice(1, 2);
