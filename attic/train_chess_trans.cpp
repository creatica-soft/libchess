...
//c++ -O3 -I /Users/ap/Downloads/libtorch/include -I /Users/ap/Downloads/libtorch/include/torch/csrc/api/include -L /Users/ap/Downloads/libtorch/lib -L /Users/ap/libchess -std=c++17 -Wl,-ltorch,-ltorch_cpu,-lc10,-lchess,-rpath,/Users/ap/Downloads/libtorch/lib,-rpath,/Users/ap/libchess -o chess_trans train_chess_trans.cpp chess_trans.cpp
...
*/
        ...
        if (debug_mode) {
            std::cout << "target_channel values (first 10): " << batch.move_type.slice(0, 0, 10) << std::endl;
            std::cout << "target_destination values (first 10): " << batch.dst_sq.slice(0, 0, 10) << std::endl;            
            // Log raw input mask channel
            std::cout << "input mask channel (batch 0, 68 + target_channel[0]): " << batch.board_moves[0][68 + batch.move_type[0].item<int64_t>()] << std::endl;            
       ...
       // Reshape target_channel for batched indexing
        auto target_indices = batch.move_type.view({batch.board_moves.size(0), 1, 1, 1}).expand({batch.board_moves.size(0), 1, 8, 8}); // Shape: [batch, 1, 1, 1]
        ...
        // Apply legal move mask to destination logits - this deflates the loss
        //destination_logits.masked_fill_(channel_mask == 0, -1e9);
        //auto dest_weights = channel_mask / (channel_mask.sum(1, true) + 1e-10); // [batch, 64]
        auto src_mask = batch.board_moves.select(1, 18).flatten(1, 2); // [batch, 64]
        //auto src_weights = src_mask / (src_mask.sum(1, true) + 1e-10); // [batch, 64]
        
        auto channel_loss = torch::nn::functional::cross_entropy(channel_logits, batch.move_type);
        //auto source_loss = torch::nn::functional::cross_entropy(source_logits, batch.src_sq, torch::nn::functional::CrossEntropyFuncOptions().weight(src_weights));
        //auto destination_loss = torch::nn::functional::cross_entropy(destination_logits, batch.dst_sq, torch::nn::functional::CrossEntropyFuncOptions().weight(dest_weights));
        ...
        auto src_count = (src_mask > 0).sum(1, true).to(torch::kFloat); // [batch, 1]
        auto src_weights = src_mask / (src_mask.sum(1, true) + 1e-10) * src_count; // Scale by count
        src_weights = src_weights + (src_mask > 0).to(torch::kFloat) * 0.01; // Min weight
        src_weights = torch::where(src_mask > 0, src_weights, torch::zeros_like(src_weights));
        
        auto dest_count = (channel_mask > 0).sum(1, true).to(torch::kFloat); // [batch, 1]
        auto dest_weights = channel_mask / (channel_mask.sum(1, true) + 1e-10) * dest_count;
        dest_weights = dest_weights + (channel_mask > 0).to(torch::kFloat) * 0.01;
        dest_weights = torch::where(channel_mask > 0, dest_weights, torch::zeros_like(dest_weights));
        
        //try this to increase performance on calc losses but lose accuracy - tried - no impact on performance
        //auto src_weights_avg = src_weights.mean(0); // [64]
        //auto dst_weights_avg = dest_weights.mean(0); // [64]
        //auto source_loss = torch::nn::functional::cross_entropy(source_logits, batch.src_sq, torch::nn::functional::CrossEntropyFuncOptions().weight(src_weights_avg));
        //auto destination_loss = torch::nn::functional::cross_entropy(destination_logits, batch.dst_sq, torch::nn::functional::CrossEntropyFuncOptions().weight(dst_weights_avg));
...
/*
        ...
        auto source_loss = -(src_target * src_prob * src_weights).sum(1).mean();
        auto destination_loss = -(dest_target * dest_prob * dest_weights).sum(1).mean();
        auto loss = 0.5 * channel_loss + 0.2 * source_loss + 0.3 * destination_loss + 0.1 * value_loss;
*/
        
        // Step 1: Mask channel_logits for illegal move types
        ...
        // Step 2: Predict channel
        auto channel_probs = torch::softmax(channel_logits, 1); // [batch, 21]
        auto predicted_channel = torch::argmax(channel_probs, 1); // [batch]
        
        // Step 3: Mask destination_logits for predicted channel
        auto predicted_indices = predicted_channel.view({batch.board_moves.size(0), 1, 1, 1}).expand({batch.board_moves.size(0), 1, 8, 8}).to(torch::kMPS); // [batch, 1, 8, 8]
        auto channel_mask = x_legal.gather(1, predicted_indices).squeeze(1).flatten(1, 2); // [batch, 64]
        source_logits.masked_fill_(channel_mask == 0, -1e6); // Mask illegal source squares
        destination_logits.masked_fill_(channel_mask == 0, -1e6); // Mask illegal destinations

        // Step 4: Predict source square
        auto source_probs = torch::softmax(source_logits, 1); // [batch, 64]
        auto predicted_source = torch::argmax(source_probs, 1); // [batch]
        
        // Step 5: Predict destination
    ...
    if (samples > 0) {
        correct /= samples;
        auto elapsed = std::chrono::duration<double>(std::chrono::steady_clock::now() - start).count();
        std::printf("Test loss: move type %.3f + source %.3f + destination %.3f + value %.6f = %.6f\n", channel_loss_total / steps, destination_loss_total / steps, source_loss_total / steps, value_loss_total / steps, total_loss_avg / steps);
...
int main(int argc, char ** argv) {
    ...
    const std::string weights_file = "chess_trans_";
    ...
    const int display_stats_every = 5;
    const int save_weights_every = 1600;
    const int bmpr_queue_len_train = 3;
    ...
    std::string f = "";
    f.append(weights_file).append("4.pt");
    if (std::filesystem::exists(f)) {
        try {
            std::cout << "Loading weights from " << f << "..." << std::endl;
            // Load traced model
            auto traced_module = torch::jit::load(f, device);
            ...
            std::cout << "Loaded weights from " << f << std::endl;
        ...
        std::cout << "Weights file " << f << " not found, using untrained model" << std::endl;
    }
    f.clear();
