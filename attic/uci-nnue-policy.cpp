...
#include "creatica-nnue-policy.hpp"
...
void uciLoop() {
  ...
  while (!quitFlag.load()) {
    if (fgets(line, sizeof(line), stdin) == NULL) {
      //EOF on stdin means the GUI is gone. Treat it as "quit" rather than exit()ing here:
      //exit() skips search_thread.join() and shutdown_thread_pool(), so static destruction
      //then runs ~vector<std::thread> over threads that are still joinable, and ~thread on a
      //joinable thread calls std::terminate. That is the "libc++abi: terminating" an engine
      //prints when its stdin is closed without a quit -- a clean exit misreported as a crash.
      log_file("uciloop(): EOF on stdin, shutting down\n");
      handleQuit();
      break;
    }
    log_file("%s", line);
  ...
  for (int i = 0; i < 5; i++) {
    ...
    for (int j = 0; j < num; j++) {
      if (i == Check) {
        ...
        //An option with no name cannot be advertised. This table is indexed by the
        //EngineSpinOptions enum, so the disabled ProbabilityMass slot is still an array entry
        //and cannot simply be deleted without shifting every index after it -- but it has no
        //name, and printing it emitted
        //    option name  type spin default 0 min 0 max 0
        //which shows up as a blank row in any GUI. Skipping unnamed entries is what the shared
        //UCI layer (uci_options.h) does, and it fixes the symptom without touching the enum.
        if (chessEngine.optionSpin[j].name[0] == '\0') continue;
...
void handleNewGame() {
  //Wait for any in-flight search before freeing the tree. cleanup() deletes every node and Edge
  //array and nulls search.root; doing that under running workers is a use-after-free. Harmless
  //with Ponder off, since then no command arrives while a search is live -- but Ponder is the
  //configuration tree reuse needs. handleStop() is the idiom, and it returns at once when
  //nothing is running.
  handleStop();
  cleanup();
  position_history.clear();
...
void setEngineOptions() {
  strcpy(chessEngine.id, "Creatica (Shared MCTS Root)");
...
int main(int argc, char **argv) {
  TB_LARGEST = 0;
  zobristHash(z);
  logfile = fopen("uci-mcts-nnue-policy.log", "a"); //was "w"
  srand(time(NULL)); 
  Stockfish::Bitboards::init();
  init_nnue();
  //Load the policy head. A missing or malformed file is not fatal: the engine falls back to
  //creatica-shared-root's own prior, which is slower but sound. Playing on with a
  //half-loaded net would be far worse than playing on without one.
  {
    const char * wp = getenv("CREATICA_POLICY") ? getenv("CREATICA_POLICY")
                                                : POLICY_WEIGHTS_DEFAULT;
    char err[512];
    if (policy_net_load(policy_net, wp, err, sizeof err)) {
      policy_enabled = true;
      //Default to prior-only. That change is one thing at a time: the learned priors
      //replace the evaluation-derived ones and nothing else moves. "full" additionally
      //stops evaluating children, which is where the speedup is and also where the 1-ply
      //lookahead is lost -- a separate question that needs its own match.
      const char * m = getenv("CREATICA_POLICY_MODE");
      policy_mode = (m && !strcmp(m, "full")) ? 2 : (m && !strcmp(m, "off")) ? 0 : 1;
      if (policy_mode == 0) policy_enabled = false;
      if (const char * t = getenv("CREATICA_POLICY_TEMP")) policy_temperature = atof(t);
      if (const char * b = getenv("CREATICA_POLICY_BLEND")) policy_blend = atof(b);
      if (const char * bs = getenv("CREATICA_BLEND_SCALE")) policy_blend_scale = atof(bs);
      if (const char * sc = getenv("CREATICA_SEED_CHILDREN")) seed_children = atoi(sc) != 0;
      if (const char * nm = getenv("CREATICA_NODE_MINIMAX"))  node_minimax  = atoi(nm) != 0;
      print("info string policy head %s loaded: %d -> %d -> %d -> %d, mode %s, temp %.3f, blend %.2f, scale %.2f, seed %d, minimax %d\n",
            wp, policy_net.in, policy_net.h1, policy_net.h2, policy_net.out,
            policy_mode == 2 ? "full" : policy_mode == 1 ? "prior" : "off", policy_temperature, policy_blend, policy_blend_scale, (int)seed_children, (int)node_minimax);
    } else {
      policy_enabled = false;
      print("info string policy head disabled (%s); using NNUE child evaluations\n", err);
    }
    if (const char * f = getenv("CREATICA_FPU")) fpu_reduction = atof(f);
  }
  setEngineOptions();
